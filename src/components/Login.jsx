  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        'https://auth-5lhr.onrender.com/api/auth/login',
        { email, password },
  ...
  fontFamily: 'Inter, Poppins, Roboto, Arial, sans-serif',
      ...
      <Paper elevation={6} sx={glassStyle}>
        <Box display="flex" justifyContent="center" alignItems="center" mb={2}>
          <LockOutlinedIcon sx={{ fontSize: 48, color: '#80deea', mb: 1 }} />
        </Box>
  <Typography variant="h4" align="center" gutterBottom sx={{ color: '#fff', fontWeight: 700, letterSpacing: 1, fontFamily: 'Poppins, Inter, Arial, sans-serif' }}>Login</Typography>
        <form onSubmit={handleSubmit}>
          ...
          <Button type="submit" variant="contained" color="primary" fullWidth disabled={loading || redirecting} sx={{ mt: 2, mb: 1, fontWeight: 600, fontSize: 18, letterSpacing: 1, background: 'linear-gradient(90deg, #80deea 0%, #b388ff 100%)', color: '#222', boxShadow: '0 2px 8px #80deea55', transition: '0.2s', '&:hover': { background: 'linear-gradient(90deg, #b388ff 0%, #80deea 100%)', color: '#000' } }}>
            {redirecting ? 'Redirecting' : (loading ? 'Logging in' : 'Login')}{(loading || redirecting) && <CircularProgress size={18} color="inherit" sx={{ ml: 1 }} />}
          </Button>
        </form>
        <Typography align="center" variant="body2" sx={{ color: '#fff', mt: 2 }}>
          Don't have an account?{' '}
          <Link href="/register" underline="hover" sx={{ color: '#80deea', fontWeight: 600, '&:hover': { color: '#b388ff' } }}>Register</Link>
        </Typography>
      </Paper>
      </Box>
    </>
